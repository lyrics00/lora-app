...
def listing_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            item.Collection_type = Collection.PUBLIC  # Default collection type.
            item.save()
...
def borrow_item_page(request, pk):
    item = get_object_or_404(Item, pk=pk)
    # Only allow borrowing if the item is checked in.
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Create a new borrow request that is pending approval.
