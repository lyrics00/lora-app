...
from .models import Item, Collection, Comment, LoRAModel
from django.contrib.auth import get_user_model
import logging
logger = logging.getLogger(__name__)
User = get_user_model()

class LoRAModelForm(forms.ModelForm):
    class Meta:
        model = LoRAModel
        fields = ['title', 'file']  # Corrected field names (replacing 'model_file' with 'file')
        widgets = {
            'title': forms.TextInput(attrs={'placeholder': "Enter the title of the LoRA model"}),
        }

class ItemForm(forms.ModelForm):
    class Meta:
        model = Item
        fields = ['title', 'description', 'location', 'lora_model']  # Added 'lora_model' field
        widgets = {
